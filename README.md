...
## 📘 개요
`MainPage`는 전체 도서 목록을 보여주는 메인 페이지 컴포넌트입니다.  
사용자가 등록한 책들이 카드 형태로 나열되며, 상단에는 페이지 제목과 등록 버튼이 위치합니다.

---

## 🧩 구성 요소
- `BookList`: 실제 책 데이터를 불러와 카드 형태로 렌더링하는 하위 컴포넌트
- `산책 가기` 버튼: `/register` 경로로 이동하여 도서를 등록할 수 있도록 유도
- 전체 레이아웃은 반응형 스타일로 구성되어 있으며, Material UI의 버튼을 사용
...
| 구성 요소 | 설명 |
|-----------|------|
| 배경색 | `#e6f7f4`의 밝은 하늘색 배경 |
| 컨테이너 | 흰색 배경, 라운드 박스, 그림자 효과 |
| 제목 | 가운데 정렬된 `"작가의 산책"` |
| 버튼 | 우측 정렬된 `산책 가기` 버튼 |
| 리스트 | `BookList`를 통해 책 목록 출력 |
